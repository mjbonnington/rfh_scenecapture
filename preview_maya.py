...
# (c) 2014-2020
#
# Generate Maya playblasts or snapshots.
# Outputs correctly formatted images that match rendered output - without
# overscan, 2D pan/zoom, etc.
# Allows custom HUD / burnin without affecting current viewport settings.
...
class Preview():
	...
	def playblast_(self):
		...
		# print(output)
		if output:
			# If only playblasting a single frame, replace hashes notation
			# with actual frame number
			if self.frRange[0] == self.frRange[1]:
				output = output.replace("####", str(self.frRange[0]).zfill(4))
		...
		# here.
			...
			if self.interruptible:
				if self.outputFormat == 'image':
					output = os.path.join(self.outputDir, '%s.#.%s' % (self.outputFile, self.compression))
				elif self.outputFormat == 'qt':
					output = os.path.join(self.outputDir, self.outputFile)
	...
	def run_playblast(self):
		""" Wrapper for Maya command to generate playblast.
		"""
		pb_args = {}
		pb_args['filename'] = os.path.join(self.outputDir, self.outputFile)
